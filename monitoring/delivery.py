...
def get_updates(token: str, offset: int, timeout: int = 25):
    ...
    if not body.get("ok"):
        print(f"[telegram] getUpdates: {body.get('error_code')} "
              f"{body.get('description')}")
        # 409 — это не сбой сети и не лечится ожиданием: либо установлен
        # вебхук, либо тот же бот опрашивается вторым экземпляром. В обоих
        # случаях кнопки мертвы полностью, а чинится это снаружи, поэтому
        # причина называется отдельной строкой с указанием, что делать.
        if body.get("error_code") == 409:
            print("[degraded] обновления забирает кто-то ещё: либо остался "
                  "контейнер прошлой сборки, либо у бота стоит вебхук. "
                  "Кнопки не работают, пока это не устранено — "
                  "перезапустите приложение или снимите вебхук.")
        return None
    return body.get("result", [])
