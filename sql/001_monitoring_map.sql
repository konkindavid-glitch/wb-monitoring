...
-- ---------------------------------------------------------------------------
--  11. ГОТОВЫЙ ТЕКСТ ПОСТА
-- ---------------------------------------------------------------------------

-- Пост пишется до показа редактору, а не после одобрения: иначе человек
-- нажимает «Запостить» под тем, чего не видел. Текст обязан сохраниться —
-- публиковать надо ровно то, что одобрили, а не переписанное заново:
-- вторая генерация даёт другой текст, и в канал уходит не согласованное.
ALTER TABLE monitoring_hits ADD COLUMN IF NOT EXISTS post_text text;
